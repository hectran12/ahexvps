...
<?php 
    ...
    $historyTiente = getAllHistoryTruCongTienByUserEmail($userInfo["email"]);
   
    $dataOrders = [];
    for ($i = 0; $i < count($historyTiente); $i++) {

        $statusCong = '';
        if($historyTiente[$i]["isAdd"] == 1) {
            $statusCong = '<span class="badge bg-success">Cộng</span>';
        } else {
            $statusCong = '<span class="badge bg-danger">Trừ</span>';
        }
        $dataOrders[] = array(
            "id" => $i,
            "info" => $historyTiente[$i]["reason"],
            "amount" => number_format($historyTiente[$i]["amount"]). ' VNĐ',
            "isCong" => $statusCong,
        );
    }
...
<script src="../js/nhat_ky_tiente.js"></script>
