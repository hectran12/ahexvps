...
<?php 
    ...
    $hoaDon = getAllHoaDonByUserEmail($userInfo["email"]);

    $dataOrders = [];
    for ($i = 0; $i < count($hoaDon); $i++) {
        $infoStr = '';
        foreach (
            json_decode(base64_decode($hoaDon[$i]["info"]), true) as $key => $value
        ) {
            $infoStr .= $key . ": " . $value . "<br>";
        }

        $status = $hoaDon[$i]["status"];
        if($status == 0) {
            $status = '<span class="badge bg-warning">Chưa thanh toán</span>';
        } else if($status == 1) {
            $status = '<span class="badge bg-success">Đã thanh toán</span>';
        } else {
            $status = '<span class="badge bg-danger">Đã hủy/Lỗi</span>';
        }
        $dataOrders[] = array(
            "id" => $i,
            "info" => $infoStr,
            "dateCreated" => $hoaDon[$i]["dateCreated"],
            "status" => $status
        );
    }

    echo '<script>var dataOrders = ' . json_encode($dataOrders) . ';</script>';
        ...
        <div class="main-content app-content">
            <div class="container-fluid">
                ...
                <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
                    <h1 class="page-title fw-semibold fs-18 mb-0">Quản lý hóa đơn</h1>
                    <div class="ms-md-1 ms-0">
                        <nav>
                            <ol class="breadcrumb mb-0">
                                <li class="breadcrumb-item"><a href="javascript:void(0);">Q.lý thanh toán</a></li>
                ...
                <table id="example" class="table text-nowrap table-secondary" style="width:100%">
    <thead>
        <tr>
            <th>ID</th>
            <th>Thông tin</th>
...
<script src="../js/hoa_don.js"></script>
